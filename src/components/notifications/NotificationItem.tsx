  ...
  return (
    <div className={`w-full ${bg} py-3 px-2 sm:px-6 flex flex-col sm:flex-row items-start gap-2 border-b last:border-b-0`}>
      <div className="relative shrink-0">
        <img src={avatar} className="w-12 h-12 rounded-full border-2 border-white" alt={name} />
        {badgeIcon && (
          <span className={`absolute -bottom-1 -right-1 rounded-full flex items-center justify-center ${badgeColor} w-7 h-7 border-2 border-white shadow`}>
            {badgeIcon}
          </span>
        )}
      </div>
