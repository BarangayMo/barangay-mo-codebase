...
import { MapPin, Building, Search, Edit, Calendar, DollarSign, Users } from "lucide-react"
    ...
    const initMaps = async () => {
      try {
        ...
        if (typeof window.google !== "undefined" && typeof window.google.maps !== "undefined") {
          ...
          console.error("❌ JobsAllPage: Google Maps API not globally available after loadGoogleMaps resolved.")
          setIsMapsReady(false)
        }
      } catch (err) {
        console.error("❌ JobsAllPage: Google Maps failed to load:", err)
        setIsMapsReady(false)
      }
    }
    initMaps()
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {selectedJob.work_approach && (
                          <Badge className={`border ${getWorkApproachColor(selectedJob.work_approach)}`}>
                            {selectedJob.work_approach}
                          </Badge>
                        )}
                      </div>
