...
const OfficialCommunity = () => {
  ...
  return (
    <>
      <Helmet>
        <title>Community - Official Dashboard</title>
      </Helmet>
      
      {/* Mobile Header */}
      {isMobile && (
        <div className="fixed top-0 left-0 right-0 z-50 bg-official text-white">
          <div className="flex items-center gap-3 px-4 py-3">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => navigate(-1)}
              className="text-white hover:bg-official-dark p-1 h-8 w-8"
            >
      ...
      <Layout hideHeader={isMobile}>
        <div className="min-h-screen bg-gray-50">
          <div className={`max-w-2xl mx-auto px-4 py-4 ${isMobile ? 'pt-16' : ''}`}>
            ...
            {!isMobile && (
              <div className="mb-6">
                <h1 className="text-2xl font-bold text-official mb-2">Community</h1>
                <p className="text-gray-600">Connect with your barangay community as an official</p>
              </div>
            )}
