...
import { loadGoogleMaps } from "@/services/googleMaps" // Import loadGoogleMaps
...
export default function OfficialJobsPage() {
  ...
  const [isMapsReady, setIsMapsReady] = useState(false) // New state for map loading
  const { user } = useAuth()
  ...
  // Load Google Maps API when the page mounts
  useEffect(() => {
    const initMaps = async () => {
      try {
        ...
        if (typeof window.google !== "undefined" && typeof window.google.maps !== "undefined") {
          ...
          console.error("❌ OfficialJobsPage: Google Maps API not globally available after loadGoogleMaps resolved.")
          setIsMapsReady(false)
        }
      } catch (err) {
        console.error("❌ OfficialJobsPage: Google Maps failed to load:", err)
        setIsMapsReady(false)
      }
    }
    initMaps()
  ...
  const getWorkApproachColor = (workApproach: string) => {
    ...
    return colors[workApproach as keyof typeof colors] || "bg-gray-100 text-gray-800 border-gray-200"
  }
      ...
      <Layout>
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="flex items-center gap-4 mb-6">
            <Button variant="ghost" onClick={() => navigate("/official-dashboard")} className="flex items-center gap-2">
                  ...
                  <div className="flex items-start gap-4 flex-1">
                    <div className="w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg flex items-center justify-center shadow-md flex-shrink-0">
                      {selectedJob.logo_url ? (
                        <img
                          src={selectedJob.logo_url || "/placeholder.svg"}
                          alt="Company logo"
                          className="w-full h-full object-cover rounded-lg"
                        />
                      ) : (
                ...
                {selectedJob.location && (
                  <div className="mb-6">
                    <div className="h-48 sm:h-64">
                      {/* Pass isMapsReady to JobMap */}
                      <JobMap location={selectedJob.location} isMapsReady={isMapsReady} />
                    </div>
                  </div>
                )}
                {/* Job Details Grid */}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mb-6">
                  ...
                  <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-base sm:text-lg font-semibold truncate">
                        {selectedJob.work_approach || "Not specified"}
                      </p>
                    </div>
                  </div>
                </div>
