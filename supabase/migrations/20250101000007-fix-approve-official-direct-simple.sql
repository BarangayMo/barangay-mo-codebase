...
-- Create a simple, robust function that handles official approval
...
BEGIN
    ...
    -- Update the official record - this is the critical part with explicit WHERE clause
    ...
    IF update_count = 0 THEN
        RETURN 'Error: Failed to update official record - no rows affected';
    END IF;
...
EXCEPTION
    WHEN OTHERS THEN
        RETURN 'Error: ' || SQLERRM || ' (SQL State: ' || SQLSTATE || ')';
END;
$$;
...
-- Test the function with a simple query to ensure it works
DO $$
BEGIN
    -- This is just a test to make sure the function can be created and called
    RAISE NOTICE 'Function approve_official_direct created successfully';
END $$; 