...
DROP FUNCTION IF EXISTS public.fix_specific_official(UUID);
DROP FUNCTION IF EXISTS public.fix_approved_officials();

-- Create a function that only creates profiles without setting user_id
CREATE OR REPLACE FUNCTION public.fix_approved_officials_profiles()
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    LOOP
        ...
        -- Update the official record (but don't set user_id yet)
    ...
    RETURN 'Fixed ' || fixed_count || ' approved officials (profiles created)';
END;
$$;

-- Grant permissions
GRANT EXECUTE ON FUNCTION public.fix_approved_officials_profiles() TO authenticated;

-- Create a function to fix a specific official
CREATE OR REPLACE FUNCTION public.fix_specific_official_profile(official_id UUID)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    IF profile_exists THEN
        RETURN 'Profile already exists for this official';
    END IF;
    ...
    RETURN 'Fixed official profile with user_id: ' || new_user_id || ' (auth user needs to be created separately)';
END;
$$;

-- Grant permissions
GRANT EXECUTE ON FUNCTION public.fix_specific_official_profile(UUID) TO authenticated; 